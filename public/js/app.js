...
// AUTH
// ═══════════════════════════════════════════════════
function getToken() { return sessionStorage.getItem('cw_token'); }
function signOut() {
...
const _usr = sessionStorage.getItem('cw_user') || '';
const _uel = document.getElementById('hdr-user');
if (_uel) _uel.textContent = _usr;
...
let isLoadingMore = false, displayedCount = 0;
const DISPLAY_BATCH = 100;

// ═══════════════════════════════════════════════════
// BOOT — load static JSON chunks, no API timeout
// ═══════════════════════════════════════════════════
async function boot() {
  ...
  try {
    ...
    if (tbody) tbody.innerHTML = '<tr><td colspan="18"><div class="empty-state"><strong>Failed to load data</strong><span>' + err.message + '</span><button class="btn" onclick="boot()" style="margin-top:8px">Retry</button></div></td></tr>';
...
function renderTable() {
  ...
  displayedCount = Math.min(DISPLAY_BATCH, tot);
  tbody.innerHTML = FILT.slice(0, displayedCount).map(makeRow).join('');
  const pgInfo = ge('pg-info');
  if (pgInfo) pgInfo.textContent = displayedCount < tot ? 'Scroll for more (' + tot.toLocaleString() + ' total)' : '';
}

function loadMoreRows() {
  if (isLoadingMore) return;
  const tot = FILT.length;
  if (displayedCount >= tot) return;
  isLoadingMore = true;
  const next = Math.min(displayedCount + DISPLAY_BATCH, tot);
  const tbody = ge('tbody');
  if (tbody) tbody.insertAdjacentHTML('beforeend', FILT.slice(displayedCount, next).map(makeRow).join(''));
  displayedCount = next;
  const pgInfo = ge('pg-info');
  if (pgInfo) pgInfo.textContent = displayedCount < tot ? 'Showing ' + displayedCount.toLocaleString() + ' of ' + tot.toLocaleString() + ' — keep scrolling' : '';
  isLoadingMore = false;
}
...
function setupScrollListener() {
  ...
  wrap.addEventListener('scroll', () => {
    if (wrap.scrollTop + wrap.clientHeight >= wrap.scrollHeight - 300) loadMoreRows();
  });
}

// ═══════════════════════════════════════════════════
// DROPDOWNS & FILTER HIGHLIGHTS
// ═══════════════════════════════════════════════════
...
function hiliteInputs() {
  ['f-mk','f-rm','f-gn','f-dw','f-yr','f-d0','f-d1',
   ...
   'f-ct0','f-ct1','f-ds0','f-ds1'].forEach(id => {
    const el = ge(id); if (el) el.classList.toggle('act', !!el.value);
  });
}
function clearAll() {
...
function doArtist() {
  ...
  const out = ge('aout');
  if (!out) return;
  if (q.length < 2) { out.innerHTML = '<div class="empty-state">Type at least 2 characters to search</div>'; return; }
  const map = {};
  for (const r of ALL) {
  ...
  if (!hits.length) { out.innerHTML = '<div class="empty-state">No matches found</div>'; return; }
  out.innerHTML = hits.map(([name, {s, rows}]) => {
    ...
    return '<div class="acard">'
      ...
      + '</div>'
      + '<div class="atblw"><table class="atbl"><thead><tr><th>Date</th><th>Market</th><th>Room</th><th>Billing</th>'
...
function buildAnalytics() {
  ...
  const aggAvg = (k, vk, top, minCnt) => {
    top = top || 12; minCnt = minCnt || 5;
