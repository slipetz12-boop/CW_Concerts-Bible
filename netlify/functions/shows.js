...
async function fetchAll(){
  ...
  const results=await Promise.all(TABS.map(async tab=>{
    try{
      const res=await sheets.spreadsheets.values.get({spreadsheetId:SHEET_ID,range:`'${tab.name}'!A:CZ`,valueRenderOption:"UNFORMATTED_VALUE",dateTimeRenderOption:"FORMATTED_STRING"});
      const parsed=parseTab(tab,res.data.values||[]);
      console.log(`Tab ${tab.name}: ${parsed.length} rows`);
      return parsed;
    }catch(err){console.error(`Tab ${tab.name} error:`,err.message);return[];}
  }));
...
exports.handler = async (event) => {
  ...
  // Parse page param — defaults to 0
  const qs = event.queryStringParameters || {};
  const page = parseInt(qs.page || "0");
  ...
  const slice = cache.data.slice(start, start + PAGE_SIZE);
